...
/**
 * Test for Databricks source plugin.
 */
public class DatabricksSourceTestRun extends DatabricksPluginTestBase {
  ...
  public void testDBMacroSupport() throws Exception {
    ...
    ETLPlugin sourceConfig = new ETLPlugin(
      DatabricksConstants.PLUGIN_NAME,
  ...
  public void testDBSource() throws Exception {
    String importQuery = "SELECT ID, NAME, SCORE, GRADUATED, SMALLINT_COL, BIG, " +
      "NUMERIC_COL, CHAR_COL, DECIMAL_COL, BYTEA_COL, DATE_COL, TIMESTAMP_COL, " +
      "TIMESTAMP_NTZ_COL, TEXT_COL, DOUBLE_PREC_COL FROM my_table " +
    ...
    Date date = new Date(CURRENT_TS);
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    LocalDate expectedDate = Date.valueOf(sdf.format(date)).toLocalDate();
    ZonedDateTime expectedTs = Instant.ofEpochMilli(CURRENT_TS).atZone(ZoneId.ofOffset("UTC", ZoneOffset.UTC));
    ...
    Assert.assertNotNull(row1.getDateTime("TIMESTAMP_NTZ_COL"));
  ...
  public void testUserNamePasswordCombinations() throws Exception {
    ...
    Map<String, String> baseSourceProps = ImmutableMap.<String, String>builder()
      ...
      .put(DatabricksConnectorConfig.HTTP_PATH, BASE_PROPS.get(DatabricksConnectorConfig.HTTP_PATH))
    ...
    ETLPlugin dbConfig = new ETLPlugin(DatabricksConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE,
    ...
    // null user name, non-null password. In Databricks, getUser() defaults to "token", so deployment succeeds.
    Map<String, String> tokenAuth = new HashMap<>(baseSourceProps);
    tokenAuth.put(DBConfig.PASSWORD, BASE_PROPS.get(ConnectionConfig.PASSWORD));
    database = new ETLStage("databaseSource", new ETLPlugin(DatabricksConstants.PLUGIN_NAME,
                                                            BatchSource.PLUGIN_TYPE, tokenAuth, null));
