...
public class BQValidation {
  ...
  private static final List<JsonObject> jsonResponse = new ArrayList<>();
      ...
      while (currentColumnCount <= columnCountSource) {
        ...
        switch (columnType) {
          ...
          case Types.TIMESTAMP:
            Assert.assertTrue(getTimestampValidation(rsSource, bigQueryData.get(jsonObjectIdx).getAsString(),
          ...
          case Types.BIGINT:
            ...
            Assert.assertEquals("Different values found for column : %s", String.valueOf(sourceBigInt),
                                String.valueOf(targetBigInt));
            break;
          ...
          case Types.INTEGER:
            ...
            Assert.assertEquals("Different %s values found for column : %s", String.valueOf(sourceInt),
          ...
          case Types.REAL:
            ...
            Assert.assertEquals("Different %s values found for column : %s", String.valueOf(sourceFloat),
  ...
  public static boolean getTimestampValidation(ResultSet rsSource, String bqDateString, String columnName,
                                               String columnTypeName) throws SQLException, ParseException {

    switch (columnTypeName) {
      case "timestamp":
        Timestamp sourceTS = rsSource.getTimestamp(columnName);
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
        Date parsedDate = dateFormat.parse(bqDateString);
        Timestamp targetTs = new Timestamp(parsedDate.getTime());
        return sourceTS.equals(targetTs);
      case "timestamptz":
        sourceTS = rsSource.getTimestamp(columnName);
        dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
        parsedDate = dateFormat.parse(bqDateString);
        targetTs = new Timestamp(parsedDate.getTime());
        return sourceTS.equals(targetTs);
      default:
        return false;
    }
