# MemSQL Query Post-run Action


Description
-----------
Runs a MemSQL query at the end of the pipeline run.
...
Example
-------
Suppose you want to delete all records from MemSQL table "userEvents" of database "prod" running on localhost, port 3306,
without authentication using driver "mariadb" if the pipeline completes successfully (Ensure that the driver for MemSQL is 
installed. You can also driver name for some specific driver, otherwise "mariadb" will be used ), 
