 ...
 */
public class MemsqlSinkTestRun extends MemsqlPluginTestBase {
  ...
  private static final Schema SCHEMA = Schema.recordOf(
    ...
    Schema.Field.of("GEOGRAPHY_COL", Schema.of(Schema.Type.STRING)),
    Schema.Field.of("GEOGRAPHYPOINT_COL", Schema.of(Schema.Type.STRING))
  );
  ...
  public void testDBSinkWithDBSchemaAndInvalidData() throws Exception {
    // MemSQL JDBC connector allows to write integer values to STRING column. Use ENUM column instead.
  ...
  private void testDBSink(String appName, String inputDatasetName, Schema schema) throws Exception {
    ...
    try (Connection conn = createConnection();
      ...
      for (StructuredRecord expected : inputRecords) {
        ...
        CustomAssertions.assertObjectEquals(expected.get("JSON_COL"), actual.getString("JSON_COL"));
        CustomAssertions.assertObjectEquals(expected.get("GEOGRAPHY_COL"), actual.getString("GEOGRAPHY_COL"));
        CustomAssertions.assertObjectEquals(expected.get("GEOGRAPHYPOINT_COL"), actual.getString("GEOGRAPHYPOINT_COL"));
        ...
        // compare seconds, since memsql 'time' type does not store milliseconds but 'LocalTime' does
  ...
  private List<StructuredRecord> createInputData() throws Exception {
    ...
    for (int i = 1; i <= 2; i++) {
      ...
      StructuredRecord.Builder builder = StructuredRecord.builder(SCHEMA)
        ...
        .set("SET_COL", "a,b,c,d")
        .set("JSON_COL", "{\"id\":1}")
        .set("GEOGRAPHY_COL", "POINT(123.45600012 78.90100000)")
        .set("GEOGRAPHYPOINT_COL", "POINT(123.45600012 78.90100000)");
