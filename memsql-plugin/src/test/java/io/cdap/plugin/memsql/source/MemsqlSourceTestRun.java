...
import io.cdap.plugin.memsql.MemsqlPluginTestBase;
...
public class MemsqlSourceTestRun extends MemsqlPluginTestBase {
  ...
  public void testDBMacroSupport() throws Exception {
    ...
    ImmutableMap<String, String> sourceProps = ImmutableMap.<String, String>builder()
      .putAll(BASE_PROPS)
      .put(MemsqlConstants.AUTO_RECONNECT, "true")
      .put(MemsqlConstants.USE_COMPRESSION, "true")
  ...
  public void testDBSource() throws Exception {
    ...
    Assert.assertEquals("{\"id\":1}", row1.get("JSON_COL"));
    Assert.assertEquals("{\"id\":1}", row2.get("JSON_COL"));
    Assert.assertEquals("POINT(123.45600012 78.90100000)", row1.get("GEOGRAPHY_COL"));
    Assert.assertEquals("POINT(123.45600012 78.90100000)", row2.get("GEOGRAPHY_COL"));
    Assert.assertEquals("POINT(123.45600012 78.90100000)", row1.get("GEOGRAPHYPOINT_COL"));
    Assert.assertEquals("POINT(123.45600012 78.90100000)", row2.get("GEOGRAPHYPOINT_COL"));
  ...
  @Test
  public void testDbSourceWithBooleanFieldInSchema() throws Exception {
    ...
    ETLPlugin sourceConfig = new ETLPlugin(
      ...
      ImmutableMap.<String, String>builder()
        .putAll(BASE_PROPS)
        .put(AbstractDBSource.DBSourceConfig.SCHEMA, schema.toString())
        .put(MemsqlConstants.AUTO_RECONNECT, "true")
        .put(MemsqlConstants.USE_COMPRESSION, "true")
        .put(AbstractDBSource.DBSourceConfig.IMPORT_QUERY, importQuery)
        .put(AbstractDBSource.DBSourceConfig.BOUNDING_QUERY, boundingQuery)
        .put(AbstractDBSource.DBSourceConfig.SPLIT_BY, splitBy)
        .put(Constants.Reference.REFERENCE_NAME, "DBSourceTestBoolean")
        .build(),
      null
    );

    String outputDatasetName = "output-dbsourcetest-boolean";
    ETLPlugin sinkConfig = MockSink.getPlugin(outputDatasetName);

    ApplicationManager appManager = deployETL(sourceConfig, sinkConfig,
                                              DATAPIPELINE_ARTIFACT, "testDbSourceWithBooleanFieldInSchema");
    runETLOnce(appManager);
  ...
  public void testUserNamePasswordCombinations() throws Exception {
    ...
    // as source
    ETLPlugin dbConfig = new ETLPlugin(MemsqlConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, baseSourceProps, null);
    ...
    database = new ETLStage("databaseSource",
                            new ETLPlugin(MemsqlConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, noUser, null));
    ...
    database = new ETLStage("databaseSource",
                            new ETLPlugin(MemsqlConstants.PLUGIN_NAME, BatchSource.PLUGIN_TYPE, emptyPassword, null));
    etlConfig = ETLBatchConfig.builder()
      .addStage(database)
      .addStage(table)
      .addConnection(database.getName(), table.getName())
  ...
  public void testNonExistentDBTable() throws Exception {
    ...
    ETLPlugin sourceBadConnConfig = new ETLPlugin(
      ...
      ImmutableMap.<String, String>builder()
        ...
        .put(AbstractDBSource.DBSourceConfig.BOUNDING_QUERY, boundingQuery)
        .put(AbstractDBSource.DBSourceConfig.SPLIT_BY, splitBy)
        .put(Constants.Reference.REFERENCE_NAME, "MemSQLTest")
        .build(),
      null);
